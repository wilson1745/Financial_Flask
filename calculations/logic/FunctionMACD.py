...
def GetHistoryDataByPeriod(DataPath, Broker, Product, Table, Start, End):
    ...
    # 找出在資料夾中的日期
    Dates = sorted([day for day in os.listdir(FilePath) if day.isdigit() == True and day >= Start and day <= End])
    # 定義要回傳的List
    Data = []
    # 透過迴圈將每天的檔案都放進該迴圈中
    for Date in Dates:
...
def getFutureDailyInfo(product, n):
    ...
    for row in tmpnightdata:
        ...
        if tmpdaydata1 != []:
            ...
            # 取得日盤的
            dayopen, dayhigh, daylow, dayclose = float(tmpdaydata1[3]), float(tmpdaydata1[4]), float(
                tmpdaydata1[5]), float(tmpdaydata1[6])
            # 判斷日夜盤高低點位
                    ...
                    for index, item in enumerate(data):
                        if data[index][:5] == row[:5]:
                            ...
                            if float(row[6]) > float(data[index][6]):
                                data[index][6] = row[6]
                            if float(row[7]) < float(data[index][7]) and float(row[7]) != 0 or float(
                                    data[index][7]) == 0:
                                data[index][7] = row[7]
                            if float(row[8]) != 0:
                                data[index][8] = row[8]
