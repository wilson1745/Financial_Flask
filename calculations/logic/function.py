...
def getDayKBar(sid, date):
    ...
    data = jcontent['data']
    # 將資料整理成可用的資訊
    data = [[i[0], float(i[3].replace(',', '')), float(i[4].replace(',', '')), float(i[5].replace(',', '')), float(i[6].replace(',', '')),
             float(i[8].replace(',', ''))] for i in data]
    return data
